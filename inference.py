...
async def step_environment(action: Dict[str, str]) -> Dict[str, Any]:
...
def build_user_prompt(step: int, observation: Dict[str, Any], last_reward: float, history: List[str]) -> str:
    ...
    request_info = []
    for req in pending_requests[:5]:  # Show first 5 requests
        request_info.append(
...
def get_model_action(client: OpenAI, step: int, observation: Dict[str, Any], 
    ...
    try:
        ...
        try:
            ...
            # Fallback: extract server_id from text
            server_id = "server_0"  # Default fallback
        ...
        return {
            "server_id": "server_0",
            "reasoning": "Model request failed, using fallback"
        }
...
async def main() -> None:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{SERVER_URL}/health") as resp:
                if resp.status != 200:
                    print(f"[ERROR] Server not available at {SERVER_URL}", flush=True)
                    return
    ...
    try:
        # Reset environment
        observation = await reset_environment()
        ...
        for step in range(1, MAX_STEPS + 1):
            ...
            # Execute action
            try:
                step_result = await step_environment(action_data)
        ...
        if rewards:
            ...
            score = max(0.0, min(1.0, score + 0.5))  # Normalize to [0, 1] range
        else:
            score = 0.0
